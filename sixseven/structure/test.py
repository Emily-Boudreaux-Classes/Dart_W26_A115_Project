...
def eos_rho_old(P, T, comp: Composition):
    """
...
def energy_generation_eps(rho, T, comp: Composition):
    ...
    os.close(devnull)
    try:
        results = burn(temps=T_arr, rhos=rho_arr, time=10000, comps=None)  # returns list of NetOut objects
    finally:
...
# ----------------------------
def stellar_structure_rhs_4eq(m, y, comp: Composition, kappa=0.4, eps_nuc=0.0, m_core=None):
    """
    4-equation stellar structure in log coordinates.
    y = [ln(r), ln(P), L, ln(T)]
    eps_nuc is precomputed from burn() at central conditions.
    Nuclear burning is only applied in the central zone (m <= m_core).
    Outside the core, dL/dm = 0 (luminosity stays constant).
    """
    ln_r, ln_P, L, ln_T = y
    r = np.exp(ln_r)
    P = np.exp(ln_P)
    ...
    # 3) Energy generation: dL/dm = eps_nuc only inside the burning core
...
def integrate_star_4eq(P_c, T_c, M, m_0, comp, kappa=0.4, core_frac=0.1, return_full=False):
    """
    Integrate 4-equation stellar structure from center to surface.
    Initial conditions: P_c, T_c at m_0.
    
    Nuclear burning (via burn()) is computed once at central conditions
    and applied only within the core (m <= core_frac * M).
    Outside the core, dL/dm = 0.
    
    Parameters
    ----------
    core_frac : float
        Fraction of total mass that defines the burning core (default 0.1 = 10%).
    """
    ...
    # Compute nuclear energy generation rate once from burn() at central conditions
    try:
        eps_nuc, mu_burn, mass_frac_0 = energy_generation_eps(rho_c, T_c, comp)
    except Exception:
    ...
    # Core boundary: burning only inside this mass coordinate
    m_core = core_frac * M
    ...
    def ode_wrapper(m, y):
        return stellar_structure_rhs_4eq(m, y, comp, kappa, eps_nuc, m_core)
...
def optimize_Pc_Tc_4eq(R_target, M, m_0, comp, 
                        ...
                        kappa=0.4, core_frac=0.1, verbose=True):
    """
    ...
    def objective(params):
        ...
        r_final = integrate_star_4eq(P_c, T_c, M, m_0, comp, kappa, core_frac)
...
if __name__ == "__main__":
    ...
    # 3) burn() call
    print("[3] Calling burn() with T={:.3e}, rho={:.3e}, time=1 ...".format(T_c_test, rho_c_test))
    rho_arr_test = np.atleast_1d(rho_c_test)
    T_arr_test = np.atleast_1d(T_c_test)
    try:
        results_test = burn(temps=T_arr_test, rhos=rho_arr_test, time=1, comps=None)
        eps_test = results_test[0].energy
        mu_burn_test = results_test[0].composition.getMeanParticleMass()
        print(f"    burn() succeeded!")
        print(f"    eps_nuc          = {eps_test:.6e} erg/g/s")
        print(f"    mu from burn()   = {mu_burn_test:.6e}")
        print(f"    composition type = {type(results_test[0].composition)}")
    except Exception as e:
        print(f"    burn() FAILED: {e}")
        eps_test = 0.0

    # 4) Initial luminosity
    L_0_test = eps_test * m_0
    print(f"[4] L_0 = eps*m_0    = {L_0_test:.6e} erg/s  ({L_0_test/3.828e33:.4e} L_sun)")

    # 5) Test ODE RHS at initial point
    y0_test = [np.log(r_0_test), np.log(P_c_test), L_0_test, np.log(T_c_test)]
    m_core_test = 0.1 * M
    rhs_test = stellar_structure_rhs_4eq(m_0, y0_test, comp, kappa=0.4, eps_nuc=eps_test, m_core=m_core_test)
    ...
    def ode_test(m, y):
        return stellar_structure_rhs_4eq(m, y, comp, 0.4, eps_test, m_core_test)
    ...
    if sol_test.success:
        ...
        # Print last values before failure
    ...
    r_final_diag = integrate_star_4eq(P_c_test, T_c_test, M, m_0, comp, kappa=0.4, return_full=False)
