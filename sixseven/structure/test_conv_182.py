...
Y_SOLAR = 0.2485
Z_SOLAR = 0.0134
# Mean molecular weight for FULLY IONIZED gas (includes free electrons)
# mu = 1 / (2X + 3Y/4 + Z/2)  — NOT the nuclear mean mass!
mu = 1.0 / (2.0*X_SOLAR + 0.75*Y_SOLAR + 0.5*Z_SOLAR)   # ≈ 0.599
...
N_SHELLS  = 50    #200            # total mass shells (log-spaced, centre to surface)
N_BURN    = 43#175                # inner shells for burn(); ~0.1 M_sun on log grid
BURN_TIME = 1e14               # burn() integration time [s]; energy output is erg/g
...
def eos_rho(P, T, comp=None):
    """EOS: Ideal gas + radiation pressure. Rejects non-finite/non-physical inputs."""
    P_arr = np.asarray(P, dtype=float)
    T_arr = np.asarray(T, dtype=float)
    bad = (~np.isfinite(P_arr)) | (~np.isfinite(T_arr)) | (P_arr <= 0) | (T_arr <= 0)
    P_safe = np.where(bad, 1.0, P_arr)
    T_safe = np.where(bad, 1.0, T_arr)
    rho = ef.simple_eos(P_safe, mu, T_safe)
    rho = np.where(bad, 1e-99, rho)
    return np.maximum(rho, 1e-99)


def energy_generation_eps(rho, T, comp=None):
    """
    Nuclear energy generation rate from gridfire burn().
    Returns eps in erg/g/s (clamped >= 0). Handles scalar and array inputs.
    """
    rho_is_scalar = np.isscalar(rho)
    T_is_scalar = np.isscalar(T)
    rho_arr = np.atleast_1d(rho)
    T_arr = np.atleast_1d(T)
    # Suppress noisy KINSol output from burn()
    ...
    eps = np.maximum(np.asarray([r.energy for r in results]) / BURN_TIME, 0.0)
...
def stellar_structure_rhs_4eq(m, y, comp: Composition,
    ...
    eps is interpolated from the m_shells/eps_shells table (from gridfire burn()).
    ...
    # 3) Energy generation (from gridfire burn() shell table)
    if m_shells is not None:
...
def integrate_star_4eq(P_c, T_c, M, m_0, comp,
    ...
    Uses gridfire burn() on the inner N_BURN shells for energy generation.
    If burn_comps is provided, it should be a list of Composition objects for
    the burn shells and will be passed through to burn().
    """
    ...
    eps_c = energy_generation_eps(rho_c, T_c, comp)
    L_0_guess = eps_c * m_0

    # Log-spaced mass shells from centre to surface
    ...
    try:
        burn_comp_list = None
        if burn_comps is not None:
            burn_comp_list = list(burn_comps[:N_BURN])
            if len(burn_comp_list) < N_BURN:
                burn_comp_list.extend(
                    [init_composition() for _ in range(N_BURN - len(burn_comp_list))]
                )
        ...
        os.close(devnull)
        try:
            results = burn(temps=T_burn, rhos=rho_burn, time=BURN_TIME,
                           comps=burn_comp_list)
        finally:
            os.dup2(saved_stdout, stdout_fd)
            os.dup2(saved_stderr, stderr_fd)
            os.close(saved_stdout)
        ...
        # erg/g -> erg/g/s, clamp non-negative
        eps_burn = np.maximum(np.array([r.energy for r in results]) / BURN_TIME, 0.0)
    except Exception as e:
        print(f"[integrate] burn() failed: {e}")
        if return_full:
            return np.inf, None
        return np.inf
...
def optimize_Pc_Tc_4eq(T_eff_target, P_phot_target, M, m_0, comp,
                        P_c_guess=2.5e17, T_c_guess=1.5e7,
                        use_convection=True, verbose=True,
                        burn_comps=None, maxiter=1000):
    """
    Optimize P_c and T_c to match photospheric T_eff and P_phot using
    the Eddington gray-atmosphere approximation.

    The integration endpoint at m=M is NOT the photosphere.  Instead:
      T_eff  = (L / (4 pi R^2 sigma))^{1/4}     (Stefan-Boltzmann)
      P_phot = (2/3) g / kappa_phot               (Eddington tau=2/3)
    where R = r(M), L = L(M), g = GM/R^2.
    """
    log_T_target = np.log10(T_eff_target)
    log_P_target = np.log10(P_phot_target)
    ...
    def objective(params):
        ...
        # Reject non-finite or non-physical states
        vals = np.array([R_surf, L_surf], dtype=float)
        if not np.all(np.isfinite(vals)) or R_surf <= 0 or L_surf <= 0:
            return 1e30

        # Eddington outer boundary: derive photospheric T_eff and P_phot
        T_eff = (L_surf / (4.0 * pi * R_surf**2 * sigma_SB))**0.25
        g_surf = G * M / R_surf**2
        # Iterate kappa-P coupling (3 iterations)
        kap = kramer_opacity(eos_rho(P_phot_target, T_eff), T_eff,
                             X_SOLAR, Y_SOLAR, Z_SOLAR)
        for _ in range(3):
            P_ph = (2.0 / 3.0) * g_surf / kap
            kap = kramer_opacity(eos_rho(P_ph, T_eff), T_eff,
                                 X_SOLAR, Y_SOLAR, Z_SOLAR)
        P_phot = (2.0 / 3.0) * g_surf / kap

        vals2 = np.array([T_eff, P_phot], dtype=float)
        if not np.all(np.isfinite(vals2)) or T_eff <= 0 or P_phot <= 0:
            return 1e30

        err_T = (np.log10(T_eff) - log_T_target)**2
        err_P = (np.log10(P_phot) - log_P_target)**2
        error = err_T + err_P

        if verbose:
            print(f"  P_c = {P_c:.3e}, T_c = {T_c:.3e}, "
                  f"R = {R_surf/R_SUN:.4f} R_sun, "
                  f"T_eff = {T_eff:.1f} K, P_phot = {P_phot:.3e}, "
    ...
    result = minimize(objective, x0, method='Nelder-Mead',
                      options={'initial_simplex': simplex,
                               'xatol': 1e-4, 'fatol': 1e-8, 'maxiter': maxiter})
...
if __name__ == "__main__":
    ...
    # 3) Energy generation at center (gridfire burn())
    m_shells_test = np.logspace(np.log10(m_0), np.log10(M), N_SHELLS)
    m_burn_edge = m_shells_test[N_BURN - 1]
    eps_c_test = energy_generation_eps(rho_c_test, T_c_test, comp)
    print(f"[3] Central eps from burn() = {eps_c_test:.6e} erg/g/s")
    ...
    def ode_pre_diag(m, y):
        return stellar_structure_rhs_4eq(m, y, comp,
                                          m_shells=np.array([m_0, M]),
                                          eps_shells=np.array([eps_c_test, eps_c_test]),
    ...
    if sol_pre.success:
        ...
        print(f"    Preliminary integration (eps=eps_c) succeeded -> P, T at {N_BURN} burn shells")
    else:
        P_burn_test = np.full(N_BURN, P_c_test)
        T_burn_test = np.full(N_BURN, T_c_test)
        print(f"    Preliminary integration failed, falling back to P_c, T_c for burn shells")
    ...
    print("=" * 80)
    print("SHOOTING METHOD: Optimizing Pc and Tc (Eddington outer boundary)")
    print(f"  use_convection = {USE_CONV},  N_SHELLS = {N_SHELLS},  N_BURN = {N_BURN}")
    print(f"  T_eff_target = {T_SURF_SUN:.1f} K,  P_phot_target = {P_SURF_SUN:.3e} dyn/cm²")
    print("=" * 80)
    ...
    # Eddington outer boundary: physical photospheric conditions from interior
    R_final_opt = r_sol[-1]
    L_final_opt = y_eval[2, -1]
    T_eff_opt = (L_final_opt / (4.0 * pi * R_final_opt**2 * sigma_SB))**0.25
    g_surf_opt = G * M / R_final_opt**2
    kap_ph = kramer_opacity(eos_rho(P_SURF_SUN, T_eff_opt), T_eff_opt,
                            X_SOLAR, Y_SOLAR, Z_SOLAR)
    for _ in range(3):
        P_phot_opt = (2.0 / 3.0) * g_surf_opt / kap_ph
        kap_ph = kramer_opacity(eos_rho(P_phot_opt, T_eff_opt), T_eff_opt,
                                X_SOLAR, Y_SOLAR, Z_SOLAR)
    P_phot_opt = (2.0 / 3.0) * g_surf_opt / kap_ph
    ...
    print(f"Central density: {eos_rho(Pc_opt, Tc_opt, comp):.4e} g/cm³")
    print(f"Central radius: {r_sol[0]:.4e} cm = {r_sol[0]/R_SUN:.6f} R_sun")
    print(f"Surface radius: {r_sol[-1]:.4e} cm = {r_sol[-1]/R_SUN:.4f} R_sun")
    print(f"Surface luminosity: {L_final_opt/L_SUN:.4f} L_sun")
    print(f"T_eff (Eddington): {T_eff_opt:.1f} K  (target: {T_SURF_SUN:.1f} K, err: {(T_eff_opt-T_SURF_SUN)/T_SURF_SUN*100:.2f}%)")
    print(f"P_phot (Eddington): {P_phot_opt:.3e} dyn/cm²  (target: {P_SURF_SUN:.3e}, err: {(P_phot_opt-P_SURF_SUN)/P_SURF_SUN*100:.2f}%)")
    print(f"T(m=M) [interior]: {T_sol[-1]:.4e} K")
    print(f"P(m=M) [interior]: {P_sol[-1]:.4e} dyn/cm²")
    print("=" * 80 + "\n")
    ...
    # --- Convection vs Radiation Plot (reuse nabla_rad_sol, nabla_ad_sol) ---
    axes[2, 0].plot(q, nabla_rad_sol, 'r-', label="nabla_rad")
    axes[2, 0].plot(q, nabla_ad_sol, 'b--', label="nabla_ad")
    ...
    is_conv_zone = (nabla_rad_sol > nabla_ad_sol).astype(float)
    ...
    with open(outpath, 'w') as f:
        ...
        f.write(f"Surface luminosity: {L_final_opt/L_SUN:.4f} L_sun\n")
        f.write(f"T_eff (Eddington): {T_eff_opt:.1f} K  (target: {T_SURF_SUN:.1f} K, err: {(T_eff_opt-T_SURF_SUN)/T_SURF_SUN*100:.2f}%)\n")
        f.write(f"P_phot (Eddington): {P_phot_opt:.3e} dyn/cm²  (target: {P_SURF_SUN:.3e}, err: {(P_phot_opt-P_SURF_SUN)/P_SURF_SUN*100:.2f}%)\n")
        f.write(f"T(m=M) [interior]: {T_sol[-1]:.4e} K\n")
        f.write(f"P(m=M) [interior]: {P_sol[-1]:.4e} dyn/cm²\n")
        f.write("=" * 80 + "\n\n")
